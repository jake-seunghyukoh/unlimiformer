...
class DatastoreBatch:
    def __init__(
        ...
        self.device = (
            ...
            else torch.device("cuda" if gpu_index else "cpu")
        )
    ...
    def search_and_reconstruct(self, queries, k):
        ...
        found_vectors = []
        for i in range(self.batch_size):
            scores, values, vectors = self.indices[i].search_and_reconstruct(
                queries[i], k
            )
            found_scores.append(scores)
            found_values.append(values)
        ...
        return (
            torch.stack(found_scores, dim=0),
            torch.stack(found_values, dim=0),
            torch.stack(found_vectors, dim=0),
        )


class Datastore:
    def __init__(
        self, dim, use_flat_index=False, gpu_index=False, verbose=False, device=None
    ) -> None:
        self.dimension = dim
        self.device = (
            device
        ...
        if not use_flat_index:
            self.index = faiss.IndexFlatIP(
                self.dimension
            )  # inner product index because we use IP attention
    ...
    def move_to_gpu(self):
        if self.use_flat_index:
            ...
            self.index = faiss.index_cpu_to_gpu(
                faiss.StandardGpuResources(), self.device.index, self.index, co
            )
    ...
    def train_index(self, keys):
        if self.use_flat_index:
            ...
            self.index = faiss.IndexIVFPQ(
                self.index, self.dimension, ncentroids, code_size, 8
            )
    ...
    def search_and_reconstruct(self, queries, k):
        ...
        )  # query vectors are same shape as "key" vectors
        scores, values, vectors = self.index.index.search_and_reconstruct(
            queries.cpu().detach(), k
        )
    ...
    def search(self, queries, k):
        ...
        if self.use_flat_index:
            if self.gpu_index:
                ...
                scores, values = faiss.knn(
                    queries, self.keys, k, metric=faiss.METRIC_INNER_PRODUCT
                )
        ...
        values = torch.where(
            torch.logical_or(values < 0, values >= self.keys.shape[0]),
