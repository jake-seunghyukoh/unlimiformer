...
def main():
    ...
    parser.add_argument(
        "--suffix", type=str, default="", help="Text added after the input."
    )
    parser.add_argument(
        "--padding_text",
        type=str,
    ...
    if len(set(unknown_args) & set(unknown_unlimiformer_args)) > 0:
        raise ValueError(
            f"Unknown arguments detected: {set(unknown_args) & set(unknown_unlimiformer_args)}"
        )
    ...
    for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
        print(
            f"=== GENERATED SEQUENCE {generated_sequence_idx + 1} (input length: {input_ids.shape[-1]}) ==="
        )
        ...
        prompt_length = (
            min(input_ids.shape[-1], model.unlimiformer.window_size())
            if unlimiformer_args.test_unlimiformer
