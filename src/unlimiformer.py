...
class Unlimiformer(Generic[ModelType]):
    def __init__(
        ...
        self.datastore_device = torch.device(
            f"cuda:{datastore_device}"
            if torch.cuda.is_available() and gpu_datastore
            else "cpu"
        )
    ...
    def inject_hooks(self, model):
        ...
        attention_layers_to_capture = self.activation_to_capture(
        ...
        for i, decoder_layer in enumerate(decoder_layers_to_run):
            ...
            # Back up the original forward function
            self.original_decoder_layer_cross_attn_forward_funcs.append(
                decoder_layer_cross_attention.forward
            )
            # Inject our hook function before the original forward function
            decoder_layer_cross_attention.forward = (
                self.create_cross_attn_pre_forward_hook(
                    decoder_layer_cross_attention.forward, i
                )
            )
    ...
    def reset_memory(self, input_ids, attention_mask):
            ...
            if self.is_encoder_decoder:
                self.datastore = [
                    DatastoreBatch(
                        ...
                        index_device=self.index_devices[0],
                    )
                ]
                self.hidden_states = [[]]
            else:
                self.datastore = [
                    DatastoreBatch(
                        ...
                        flat_index=self.flat_index,
                        gpu_index=self.gpu_index,
                        index_device=self.index_devices[i % len(self.index_devices)],
                    )
        ...
        for (
            ...
            logger.info(
                f"Encoding {context_start_ind} to {context_end_ind} out of {input_ids.shape[-1]}"
            )
            ...
            with torch.inference_mode():
                _ = self.model(
                    chunk, attention_mask=chunk_attention_mask, labels=dummy_labels
            ...
            if self.use_datastore:
                ...
                mask_device = chunk_attention_mask.device  # !
                to_add = [
                    state[:, update_start_ind:update_end_ind].detach().to(mask_device)
                    for state in hidden_states_to_index
                ]
                ...
                if not self.reconstruct_embeddings:
                    ...
                    if not self.gpu_datastore:
                        to_add_embeddings = [
                            states.cpu() for states in to_add_embeddings
                        ]
                    ...
                    for i, layer_states in enumerate(to_add_embeddings):
                        ...
                        self.hidden_states[i].append(
                            layer_states.to(self.datastore_device)
                        )
            ...
            if (not self.use_datastore) or self.test_datastore:
                ...
                for i, (layer_key, layer_value) in enumerate(zip(key, value)):
                    self.prompt_keys[i].append(
                        layer_key
                    )  # (num_layers, batch, head, chunked_source_len, dim)
                    self.prompt_values[i].append(
                        layer_value
                    )  # (num_layers, batch, head, chunked_source_len, dim)
        ...
        if self.use_datastore:
            ...
            if not self.reconstruct_embeddings:
                ...
                for i in range(len(self.hidden_states)):
                    concat_hidden_states.append(
                        torch.cat(self.hidden_states[i], axis=1)
                    )
                    self.hidden_states[i] = None
                self.hidden_states = concat_hidden_states
            for datastore, layer_hidden_states in zip(
                self.datastore, self.hidden_states
            ):
                datastore.train_index(layer_hidden_states)
        if (not self.use_datastore) or self.test_datastore:
            for i, (layer_keys, layer_values) in enumerate(
                zip(self.prompt_keys, self.prompt_values)
            ):
    ...
    def create_cross_attn_pre_forward_hook(
        self, original_cross_attn_forward_func, cur_layer_num
    ):
        def attention_pre_forward_hook(
            ...
            if self.model.training:
                ...
                result = original_cross_attn_forward_func(
                    hidden_states=hidden_states,
                    attention_mask=attention_mask,
                    *args,
                    **kwargs,
    ...
    def attention_forward_hook(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            if self.use_datastore:
                ...
                # k_proj in attention?
                datastore_index = (
                    0 if self.is_encoder_decoder else self.cur_decoder_layer_index
                )
                attention_layer_list = self.get_kv_projections(
                    self.layer_begin, self.layer_end
                )
                ...
                if self.reconstruct_embeddings:
                    ...
                    _, top_search_key_indices, embeddings = self.datastore[
                        datastore_index
                    ].search_and_reconstruct(datastore_query, k=topk)
                else:
        ...
        if self.test_datastore:
            ...
            assert (
                torch.mean(
                    torch.isclose(
                        correct_keys, retrieved_keys, rtol=1e-3, atol=1e-3
                    ).float()
                )
                > 0.99
            )
            assert (
                torch.mean(
                    torch.isclose(
                        correct_values, retrieved_values, rtol=1e-3, atol=1e-3
                    ).float()
                )
                > 0.99
            )
        ...
        self.cur_layer_key_value_placeholder[0] = torch.cat(
            [retrieved_keys, self.cur_layer_key_value_placeholder[0][:, :, topk:]],
            dim=-2,
        )
        self.cur_layer_key_value_placeholder[1] = torch.cat(
            [retrieved_values, self.cur_layer_key_value_placeholder[1][:, :, topk:]],
            dim=-2,
        )
        return
...
class UnlimiformerLLaMa(Unlimiformer[LlamaModel]):
    ...
    def preprocess_query(self, query, k_proj_weight):
        ...
        # cos = cos[:, :, -1].to(query.device)  # [1, 1, dim]
        # sin = sin[:, :, -1].to(query.device)  # [1, 1, dim]
        ...
        query = query.to(k_proj_weight.device)

        k_proj = k_proj_weight.view(
            1, self.num_heads, query.shape[-1], k_proj_weight.shape[0]
        ...
        datastore_query = torch.matmul(
            datastore_query, k_proj + k_proj_rotated
    ...
    def post_process_retrieved(
        self, embeddings, k_proj_layer, v_proj_layer, top_search_key_indices
    ):
        ...
        if k_proj_layer.bias is not None:
            k_bias = (
                k_proj_layer.bias.view(1, self.num_heads, embed_dim // self.num_heads)
                .unsqueeze(-2)
                .unsqueeze(0)
            )
        ...
        if v_proj_layer.bias is not None:
            v_bias = (
                v_proj_layer.bias.view(1, self.num_heads, embed_dim // self.num_heads)
                .unsqueeze(-2)
                .unsqueeze(0)
            )

        embeddings = embeddings.to(k_weight.device)

        # new_keys, new_values: (batch, beam, head, encoder_len, attn_dim)
        retrieved_keys = (
            torch.matmul(embeddings, k_weight) + k_bias
        )  # (beam, head, encoder_len, embed_dim)
        retrieved_values = (
            torch.matmul(embeddings, v_weight) + v_bias
        )  # (beam, head, encoder_len, embed_dim)

        attention = self.model.base_model.layers[-1].self_attn
        cos, sin = attention.rotary_emb(
            retrieved_values, seq_len=self.hidden_states[0].shape[1]
        )
        cos = cos.squeeze(1).squeeze(0).to(k_weight.device)  # [seq_len, dim]
        sin = sin.squeeze(1).squeeze(0).to(k_weight.device)  # [seq_len, dim]
        if self.prompt_input_ids.shape[1] > self.actual_model_window_size:
            ...
            scaled_key_indices = (
                (top_search_key_indices / self.prompt_input_ids.shape[1])
                * self.actual_model_window_size
            ).int()
        else:
